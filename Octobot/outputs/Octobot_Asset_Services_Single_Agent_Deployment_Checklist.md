...
1. `Hi`
   ...
   - Final status is `SUCCESS` with empty arrays and `error: ""`.
2. `features`
   - No Asset Services business tool is called.
...
4. A category-conflict lookup
   ...
   - `error` is `""` and no contradictory record is rendered.
...
- all eight single-agent finalizer properties are present, including `error`;
- `attributes`, `tables`, `options`, and `toolsUsed` are arrays;
- attribute values, table columns, and table cells are strings;
- `error` is `""` for non-failed responses and a non-empty safe string for
  `FAILED`; it is never `null`;
- every populated option uses strings for `source_table` and `value_column` and
  an array of strings for `display_columns`, with typed empty values when those
  links do not apply;

...
| TM | `query_tm_eod_transactions` | EOD security transaction data for an account and date | Confirmed from supplied TM screenshot |
| TM | `query_tm_current_transactions` | Current/recent security transaction activity | Confirmed from supplied TM screenshot |
| TM | `query_tm_settlement_instructions` | Current transaction activity requiring settlement-instruction detail | Confirmed from supplied TM screenshot |

Confirmed TM metadata visible in the supplied screenshots:

| Tool | Catalog | Source name | English name | Portable ID |
| --- | --- | --- | --- | --- |
| `query_tm_eod_transactions` | `CDS2_ALL` | `VIEW_CDS2_Transactions` | EOD Security Transactions Dataset | `f35ab57f-9213-453f-92bd-7c0136844f58` |
| `query_tm_current_transactions` | `CDS2_RT` | `TCD2_JOINS_IOD` | Current Securities Transactions | `fdbc0b64-8bb3-46e2-b4fa-dccccd4cc377` |
| `query_tm_settlement_instructions` | `CDS2_RT` | `TCD2_JOINS_IOD_SFE` | Current Securities Transactions with settlement instruction details | `c9872ea4-dd87-4721-baa0-00aff7387aaf` |
...
```mermaid
flowchart TB
  subgraph ONLINE["1. Online request and data retrieval"]
    direction LR
    U[User] --> R[Root Agent]
    R -->|AS or BOTH| AS[AS Specialist Sub-Agent]
    R -->|TM or BOTH| TM[TM Specialist Sub-Agent]
    AS <--> AST[Two AS service tools]
    TM <--> TMT[Three TM service tools]
    AST <--> RT[Shared MCP Runtime]
    TMT <--> RT
    RT <--> API[Provider APIs]
  end

  subgraph STARTUP["2. Startup-only metadata initialization"]
    direction LR
    DB[(PostgreSQL: two tables)] --> ML[Startup Loader and Validator]
    ML --> REG[Immutable In-Memory Registry]
    REG -. in-process lookup .-> RT
  end

  subgraph RESPONSE["3. Response composition"]
    direction LR
    SR[Structured specialist responses] --> F[Formatter Agent]
    F --> V[Final JSON Validator]
    V --> OUT[User]
  end
```

The three bands intentionally separate concerns. Online retrieval is the only provider request path. PostgreSQL is read only during startup initialization. After tool execution, the AS and TM Specialist Sub-Agents place their normalized results in the structured response envelope used by the Formatter Agent. The formatter never receives raw provider data directly and never calls a tool.
...
| Historical/as-of EOD transactions | `query_tm_eod_transactions` |
| Current/recent transaction activity | `query_tm_current_transactions` |
| Settlement-instruction fields | `query_tm_settlement_instructions` |
...
```python
TOOL_NAME = "query_tm_current_transactions"


@mcp.tool(name=TOOL_NAME)
async def query_tm_current_transactions(
...
| TM Specialist Sub-Agent | `query_tm_eod_transactions`, `query_tm_current_transactions`, `query_tm_settlement_instructions` |
...
{
  "requestId": "req_123",
  "toolName": "query_tm_current_transactions",
...
2. TM specialist selects `query_tm_current_transactions` only.
